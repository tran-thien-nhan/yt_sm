...
const TextSummarizer = () => {
    ...
    const handleExcelAnalysis = (analysis) => {
        ...
        setInputText(`Excel Analysis:\n${JSON.stringify(analysis, null, 2)}`);
    };

    const handleTrizAnalysis = async () => {
        if (!summary) {
        ...
        setIsLoading(true);
        try {
            // Chuẩn bị dữ liệu TRIZ để đưa vào prompt
            const trizDataString = JSON.stringify(trizData);

            // Tạo prompt với dữ liệu TRIZ
            ...
            1. Các tham số R&D Spec/Capability/Means liên quan nhất, dịch sang tiếng việt.
            2. Các nguyên tắc sáng tạo (inventive principles) phù hợp nhất, dịch sang tiếng việt.
            3. Đề xuất thật nhiều giải pháp sáng tạo dựa trên các nguyên tắc đã chọn, dịch sang tiếng việt.
            ...
            Vui lòng trả lời theo định dạng JSON như sau:
            {
                ...
                "principles": [{"number": số nguyên tắc, "name": "tên nguyên tắc"}, ...],
                "solutions": ["giải pháp 1", "giải pháp 2", "giải pháp 3"]
            }`;

            const trizResponse = await axios.post(
                `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${api}`,
                {
            ...
            console.log(trizResponse.data); // Log the entire response for debugging
            ...
            if (jsonMatch && jsonMatch[1]) {
                trizAnalysisResult = JSON.parse(jsonMatch[1]);
            } else {
                throw new Error("Không tìm thấy JSON hợp lệ trong phản hồi");
            }

            // Tạo nội dung phân tích TRIZ
                        ...
                        {trizAnalysisResult.principles.map((principle, index) => (
                            <ListItem key={index}>
                                <ListItemText
                                    ...
                                    secondary={trizData.inventive_principles.find(p => p.number === principle.number)?.description}
                                />
                            </ListItem>
                        ))}
                    </List>
